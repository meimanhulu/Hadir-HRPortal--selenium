...
public class LaporanSemuaScreen {
        ...
        public void inputName(String name) {
                ...
                searchInputElement.sendKeys(Keys.CONTROL, "a"); // Pilih semua teks
                searchInputElement.sendKeys(Keys.BACK_SPACE); // Hapus teks yang dipilih
        ...
        public boolean verifyDepartmentFilterResult(String expectedDepartment) {
                for (int i = 0; i < getNumberOfTableRows(); i++) {
                        By departmentDataFromTable = By.xpath("//tbody/tr[" + (i + 1) + "]/td[3]/h6[1]");
        ...
        public boolean verifyDateRange(String startDate, String endDate) {
                ...
                for (int i = 0; i < getNumberOfTableRows(); i++) {
                        By dateColumn = By.xpath("//tbody/tr[" + (i + 1) + "]/td[4]"); // Sesuaikan dengan kolom tanggal
                        WebElement dateElement = DriverWaitHelper.DRIVER_WAIT
                                        .until(ExpectedConditions.elementToBeClickable(dateColumn));

                        try {
                                Date reportDate = sdf.parse(dateElement.getText());
        ...
        public boolean isDateRangeDisplayed(String startDate, String endDate) {
                SimpleDateFormat dateFormat = new SimpleDateFormat("dd/MM/yyyy", Locale.ENGLISH);

                for (int i = 0; i < getNumberOfTableRows(); i++) {
                        By dateDataFromTable = By.xpath("//tbody/tr[" + (i + 1) + "]/td[4]"); // Asumsi tanggal ada di
                                                                                              // kolom ke-4
                        WebElement dateElement = DriverWaitHelper.DRIVER_WAIT
                                        .until(ExpectedConditions.visibilityOfElementLocated(dateDataFromTable));

                        try {
                                Date dateFromTable = dateFormat.parse(dateElement.getText());
                                Date start = dateFormat.parse(startDate);
                                Date end = dateFormat.parse(endDate);

                                if (dateFromTable.before(start) || dateFromTable.after(end)) {
                                        return false;
                                }
                        } catch (ParseException e) {
                                e.printStackTrace();
        ...
        public boolean isNameDisplayed(String expectedName) {
                for (int i = 0; i < getNumberOfTableRows(); i++) {
                        By nameDataFromTable = By.xpath("//tbody/tr[" + (i + 1) + "]/td[2]/h6[1]"); // Asumsi nama ada
                                                                                                    // di kolom ke-2
                        WebElement nameElement = DriverWaitHelper.DRIVER_WAIT
                                        .until(ExpectedConditions.visibilityOfElementLocated(nameDataFromTable));

                        if (!nameElement.getText().toLowerCase().contains(expectedName.toLowerCase())) {
                                return false;
                        }
                }
                return true;
